...
/* Use IR2..IR7 for steering */
#define STEER_SENSOR_MASK       ((1U << 1) | (1U << 2) | (1U << 3) | \
                                 (1U << 4) | (1U << 5) | (1U << 6))

/* Ultrasonic */
#define US_TIMEOUT_US           30000U
...
static void servo_set_angle(float angle)
{
  ...
  servo_set_pulse_us((uint16_t)(pulse_us + 0.5f));
}
...
static void motor_set_both(uint8_t value)
{
  motor_set_raw(MOTOR_LEFT_CHANNEL, value);
  motor_set_raw(MOTOR_RIGHT_CHANNEL, value);
}
...
static bool ir_read_black(uint8_t idx)
{
  ...
  GPIO_PinState state = HAL_GPIO_ReadPin(ir_ports[idx], ir_pins[idx]);
...
static uint32_t us_echo_pulse_us(uint32_t timeout_us)
{
  ...
  {
    if ((micros() - start_wait) > timeout_us)
    {
      return 0U;
    }
  }
...
static uint32_t us_read_distance_mm(void)
{
  ...
  uint32_t pulse_width_us = us_echo_pulse_us(US_TIMEOUT_US);
  if (pulse_width_us == 0U)
  {
    return 0U;
  }

  return (pulse_width_us * 343U) / 2000U;
}
...
static void steering_bang_bang_6(uint8_t ir_mask)
{
  ...
  float target_angle = SERVO_STRAIGHT_ANGLE;
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, MOTOR_L_DIR_Pin | MOTOR_R_DIR_Pin | US_trig_Pin, GPIO_PIN_RESET);
  ...
  GPIO_InitStruct.Pin = MOTOR_L_DIR_Pin | MOTOR_R_DIR_Pin | US_trig_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
